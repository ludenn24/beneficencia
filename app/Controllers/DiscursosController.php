...
Class DiscursosController extends Controller {
    ...
    public function Listar($request, $response, $args) {
        try {
            $data = Discursos::select(DB::raw('DATE_FORMAT(created_at, "%d-%b-%Y") as formated'), 'tb_discursos.*')
    ...
    {
        $codigo = $args['cod'];
        $noticia = Discursos::select(DB::raw('DATE_FORMAT(created_at, "%d-%b-%Y") as formated'), 'tb_discursos.*')
            ->where('codigo', $codigo)
            ->first();
         $totalnoticia = Discursos::where('codigo', '!=', $codigo)
                 ->where('estado', 1)->get();
    ...
    public function Editar($request, $response, $args) {
        try {
            ...
            $data = Discursos::where('codigo', '=', $codigo)->get();
           ...
           $nombrerec1 = basename($_FILES["fotoeditar"]["name"]);
            ...
            if ($tiporec1 != 'png' and
                ...
                $mensaje['message'] = $this->show('1', 'Solo se permiten subir archivos de menos de 25 Megabytes.');
                 ...
                 $codigo=$request->getParam('codigo');
                 Discursos::where('codigo', '=', $codigo)->update([
