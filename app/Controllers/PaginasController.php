...
Class PaginasController extends Controller {
    ...
    public function ListPagina($request, $response, $args) {
        try {
            $data = Pagina::select('codigo','nombre','portada','title','url_canonica','estado')->orderBy('codigo', 'DESC')->get();
            $arreglo["data"] = $data;
    ...
    public function UptPrevia($request, $response, $args)
    {
           $carpeta = "uploads/paginas/";
           $fecha_actual = date('Y-m-d-H-i-s');
           $nombrerec1 = basename($_FILES["portadaeditar"]["name"]);
    ...
    public function UptPreviaNuevo($request, $response, $args)
    {
           $carpeta = "uploads/paginas/";
           $fecha_actual = date('Y-m-d-H-i-s');
           $nombrerec1 = basename($_FILES["portada"]["name"]);
           $ext = pathinfo($nombrerec1, PATHINFO_EXTENSION);
            if ($nombrerec1) {
                
            $srcec1 = $carpeta . $fecha_actual . '.' . $ext;
            $tiporec1 = basename($_FILES["portada"]["type"]);
            $sizerec1 = basename($_FILES["portada"]["size"]);
            $moveeerec1 = $_FILES["portada"]["tmp_name"];
            if ($tiporec1 != 'png' and $tiporec1 != 'jpg' and $tiporec1 != 'jpeg') {
                        $mensaje['response'] = 'error';
                        $mensaje['message'] = 'Solo se permiten archivos JPG, JPEG O PNG';
            } elseif ($sizerec1 >= 262144000) {
